...
#label_index = {'None-cancer':0, 'bowens1':1, 'bcc1':3}
label_auc = {'None-cancer':[], 'bowens1':[], 'scc1':[], 'bcc1':[]}
#label_auc = {'None-cancer':[], 'bowens1':[], 'bcc1':[]}
macro_avg_auc = []
...
for checkpoint_file in glob.glob('../*.pt'):
    ...
    test_image_data = SlideDataset.SlideDataset('test', '../../../settings/new_all_data_with_specimen_id.csv',
        '../../../gen_tiles/NEW/accumulate_1000/tiles_40X_1000/',
    ...
    conf_matrix = metrics.confusion_matrix(ground_truth_lt, pred_index_lt, labels=(0,1,2,3))
    ...
    overall_pred.append(pred_lt)
    overall_truth.append(ground_truth_lt)
    overall_img_name_lt.append(img_name_lt)

    skplt.metrics.plot_roc(ground_truth_lt, pred_lt)
    plt.savefig(f'{checkpoint_name}_ROC.png')
    plt.close()

    with open(f'{checkpoint_name}_pred.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['slide_name', 'ground_truth', 'none_cancer', 'bowens1', 'scc1', 'bcc1'])
        for i in range(len(img_name_lt)):
            t_img_name = img_name_lt[i]
            t_ground_truth = ground_truth_lt[i]
            t_pred = list(pred_lt[i])
            row = [t_img_name, t_ground_truth] + t_pred
            writer.writerow(row)


    #skplt.metrics.plot_precision_recall(ground_truth_lt, pred_lt)
    #plt.savefig(f'{checkpoint_name}_PR.png')
    #plt.close()

for i in range(len(overall_truth)-1):
    assert np.all(overall_truth[i] == overall_truth[i+1])
    assert np.all(overall_img_name_lt[i] == overall_img_name_lt[i+1])

overall_pred = np.array(overall_pred)
overall_pred = np.mean(overall_pred, axis=0)

skplt.metrics.plot_roc(overall_truth[0], overall_pred)
plt.savefig(f'ensemble_ROC.png')
plt.close()

with open(f'ensemble_pred.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(['slide_name', 'ground_truth', 'none_cancer', 'bowens1', 'scc1', 'bcc1'])
    for i in range(len(overall_img_name_lt[0])):
        t_img_name = overall_img_name_lt[0][i]
        t_ground_truth = overall_truth[0][i]
        t_pred = list(overall_pred[i])
        row = [t_img_name, t_ground_truth] + t_pred
        writer.writerow(row)


#skplt.metrics.plot_precision_recall(overall_truth[0], overall_pred)
#plt.savefig(f'ensemble_PR.png')
#plt.close()
