...
class SlideDataset(data.Dataset):
    ...
    def __init__(self, stage, label_file, dir, transform, fold, inst_num=10):
        ...
        while True:
            ...
            if sid not in sid2pid:
                sid2pid[sid] = pid
            else:
                assert sid2pid[sid] == pid
            ...
            if pid not in self.specimen_label:
                self.specimen_label[pid] = label
            else:
                #assert self.specimen_label[pid] == label
                if self.specimen_label[pid] != label:
                    print(f'pid: {pid}')
        fin.close()
        ...
        for img_name in ids:
            ...
            #specimen_id = array[0]
            if slide_id not in sid2pid:
                continue

            specimen_id = sid2pid[slide_id]
        ...
        self.specimen_id = []
        for specimen_id in mask_id:
            if specimen_id in self.specimen_label and len(self.specimen_img[specimen_id]) > 20:
